 ...
 * stock); platform/category/status colors use their own semantic
 * palette so red keeps its meaning as "needs attention".
 * ------------------------------------------------------------------- */
const ACCENT = "#9A1B1B";
...
// fills — see Production.jsx for the same convention.
...
function StatCard({ label, value, sub, color = "text-gray-900", dot, loading }) {
  return (
...
function SectionHeading({ title, subtitle }) {
  return (
    <div className="flex items-center gap-3 pt-2">
      <h2 className="text-sm font-semibold text-gray-800 whitespace-nowrap">{title}</h2>
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard label="Total stock" value={fmt(totals?.total)} sub="units on hand" loading={loading} />
        ...
        <StatCard label="Out of stock" value={fmt(outOfStockCount)} sub="products at zero" color="text-red-700" loading={loading} />
      </div>

      {/* Stock by platform (primary) + Stock by category (compact) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`lg:col-span-2 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-4">Stock by platform</h2>
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
              ...
              <div>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between">
                  ...
                  <span className="text-sm font-semibold" style={{ color: ACCENT }}>{fmt(totals?.total)} units</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className={`lg:col-span-1 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-1">Stock by category</h2>
          {loading ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`lg:col-span-2 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-4">Top stocked items</h2>
          {loading ? (
            <Skeleton className="h-56 w-full" />
          ) : topMovers.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={Math.max(220, topMovers.length * 44)}>
              ...
              >
                ...
                >
                  {topMovers.map((m) => (
                    <Cell key={m.name} fill={PLATFORM_COLORS[m.platform] ?? ACCENT} />
                  ))}
                  <LabelList
                    dataKey="qty"
                    position="right"
        ...
        <div className={`lg:col-span-1 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-4">
            Low stock alerts
            {!loading && (
              <span className="ml-2 px-2 py-0.5 bg-red-50 text-red-700 text-xs rounded font-medium">
                {lowStock.length}
              </span>
            )}
          </h2>
                  ...
                  <div key={p.code}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`px-1.5 py-0.5 text-xs rounded font-medium shrink-0 ${
                          isOut ? "text-red-700 bg-red-50" : "text-amber-700 bg-amber-50"
                        }`}
                      >
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${pct}%`, background: isOut ? ACCENT : "#D97706" }}
                      />
                    </div>
                  </div>
                );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={`${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-4">Items needing attention</h2>
          {loading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => <Skeleton key={i} className="h-8 w-full" />)}
            </div>
            ...
            <div className="divide-y divide-gray-100">
              {slowMovers.map((m) => (
                <div key={m.name} className="flex items-center justify-between py-2.5">
                  ...
                  <span className="px-2 py-0.5 text-xs rounded font-medium bg-amber-50 text-amber-700">
                    {m.days}d
                  </span>
                </div>
              ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Total materials" value={fmt(materialStats.total)} sub="tracked SKUs" loading={loading} />
        <StatCard label="Estimated value" value={fmtPeso(materialStats.value)} sub="stock × unit cost" color="text-emerald-700" loading={loading} />
        <StatCard label="Low stock" value={fmt(materialStats.lowStockCount)} sub="need reordering soon" color="text-amber-700" loading={loading} />
        <StatCard label="Out of stock" value={fmt(materialStats.outOfStockCount)} sub="blocking production" color="text-red-700" loading={loading} />
      </div>

      {/* Value by category + Stock status */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`lg:col-span-2 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-4">Material value by category</h2>
          {loading ? (
            <Skeleton className="h-56 w-full" />
        ...
        <div className={`lg:col-span-1 ${CARD} p-5`}>
          <h2 className="text-sm font-semibold text-gray-800 mb-1">Stock status</h2>
          {loading ? (
            <Skeleton className="h-40 w-full" />
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between rounded-md border px-3 py-2 ${
                    isOut ? "bg-red-50/40 border-red-200" : "bg-amber-50/40 border-amber-200"
                  }`}
                >
                  ...
                  <span
                    ...
                    style={{ color: isOut ? ACCENT : "#D97706" }}
                  >
        ...
        >
          ...
          >
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full" style={{ background: breakdown.color }} />
